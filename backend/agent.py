...
def rag_search_tool(query: str) -> str:
    """Top-K chunks from vector database (empty string if none)"""
    try:
...
#Node 1 : Router(descision node)

def router_node(state: AgentState)->AgentState:
  print("Entering router node")
  #extract query from state
  query=next((m.content for m in reversed(state["message"]) if isinstance(m,HumanMessage)),"")
  ...
  # web_search_enabled = config.get("configurable", {}).get("web_search_enabled", True) config import not implemented yet
...
# Node 2 : Rag lookup node
# def rag_node(state: AgentState,config:RunnableConfig) -> AgentState: config not implemented yet
def rag_node(state: AgentState) -> AgentState:
  print("Entering RAG node")
  query = next((m.content for m in reversed(state["messages"]) if isinstance(m, HumanMessage)), "")
  web_search_enabled = state.get("web_search_enabled", True) # <-- CHANGED LINE
  print(f"Router received web search info : {web_search_enabled}")
  print(f"RAG query: {query}")

  chunks=rag_search_tool.invoke(query)

  # logic to hadle chunks
  ...
  # Decide next route based on sufficiency and web search info
